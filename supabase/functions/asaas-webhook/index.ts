...
async function handleClube(supa: any, event: string, payment: any): Promise<string> {
  ...
  // 16/09/2026: CICLO de 30 dias a partir do pagamento confirmado — este webhook é o
  // ÚNICO lugar que cria ciclo/crédito. inicio = agora (confirmação), fim = +30 dias,
  // total = teto do plano. Idempotente pelo id do pagamento (retry não cria 2º ciclo).
  const inicio = new Date();
  const fim = new Date(inicio.getTime() + 30 * 24 * 60 * 60 * 1000);
  const { error: cicloErr } = await supa.from("clube_creditos").upsert(
    {
      assinante_id: reg.id,
  ...
  if (cicloErr) console.error("clube: ciclo não criado:", cicloErr);
